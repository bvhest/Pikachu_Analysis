...
Available information in CCR;
  ...
  2. Branch: branch-id, tree-id, ssid (id from Pikachu)
  3. Tree: tree-id
...
Based on grouping on the url, there are `r nrow(awards.c)` unique assets. This means that 
  ...
  - the number of unique url's is higher than the number of unique id's (!),
  - a number of assets/awards are related to multiple products. These assets are:

```{r data_count3, echo=FALSE}
awards.c %>%
  dplyr::group_by(id) %>%
  dplyr::summarise(count = n_distinct(parent_id)) %>%
  dplyr::filter(count > 1) %>%
  dplyr::arrange(id, parent_id) %>%
  kableExtra::kable(caption="Assets (url's) used with different products") %>%
  kableExtra::kable_styling()
```
