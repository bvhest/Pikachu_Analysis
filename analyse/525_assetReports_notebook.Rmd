...
where scr.pm_name in (select pm_name 
  ...
  and dt.aa_id    = aa.aa_id
group by sc.dt_cd, dt.dt_restricted_ind, aa.aa_desc
...
```{r}
# /*
# Select application area and source-systems for unique doctypes related to IPL products
# */
qry <- 
  "select unique dt.dt_cd as doc_type
     , decode(dt.dt_restricted_ind, 'Y', 'restricted', 'public') as availability
     , aa.aa_desc as application_area
     , dtr.dtr_source_system as source_system
from m_document_type dt
   , m_application_area aa
   , m_document_tree dtr
where dt.aa_id    = aa.aa_id
  and dt.dtr_id   = dtr.dtr_id(+)
order by doc_type"
...
DBI::dbGetQuery(conn = pr.jdbcConnection, 
                statement = qry ) %>%
  kableExtra::kable(caption="Doc-types by application area and source-systems") %>%
  kableExtra::kable_styling()
```
