...
<xsl:stylesheet version="2.0"
  ...
  <xsl:variable name="v_reload" select="if( $reload='true') then 'true' else 'false'"/>
		...
		<root>
			...
			<xsl:for-each-group select="$list/list/entry" group-by="@filetype">
				<xsl:for-each select="current-group()">
					...
					<cinclude:include src="cocoon:/createEntryRecords/{$ct}/{$timestamp}/{@filebase}.{@fileExt}?reload={$v_reload}"/>
				</xsl:for-each>
			</xsl:for-each-group>
		</root>
	</xsl:template>
