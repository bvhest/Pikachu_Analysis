      ...
      <entry includeinreport="yes">
        <root>
          <xsl:for-each select="dir:directory/dir:file">
            <xsl:variable name="id" select="replace(substring-before(substring-after(@name,'__'),'.xml'),'_','/')"/>
            <xsl:choose>
              <xsl:when test="not(key('k_octls',$id))">
                <!-- cache file exists but octl is missing in the db -->
                <delete name="{@name}">
                  <shell:delete>
