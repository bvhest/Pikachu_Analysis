...
<xsl:preserve-space elements="*"/>
        ...
        <xsl:if test="dir:file/dir:xpath/entry">
                <document>
                    <email:sendmail>
                        ...
                        <email:subject><xsl:value-of select="concat('Warning: Translation Import Errors (instance:',substring-before(substring-after($server,'http://'),':'),') - autogenerated email sent by Pika Chu')"/></email:subject>                        
                                <email:body mime-type="text/plain">                                
The following length errors were encountered during Translation Import processing:
<xsl:for-each select="dir:file[dir:xpath/entry]">
         File name: <xsl:value-of select="@name"/>
   
   
  <xsl:for-each select="dir:xpath/entry">
