...
<xsl:stylesheet version="2.0" 
  ...
  <xsl:template match="/">
    <source:write xmlns:source="http://apache.org/cocoon/source/1.0">
      <source:source>
        <xsl:value-of select="concat($dir,'/logs/ErrorReport_',$locale,'_',$timestampnumeric,'.xml')" />
      </source:source>
      <source:fragment>
        <report runTimestamp="{$timestamp}">
          <error>
            <message>Too many changed products for <xsl:value-of select="$locale"/></message>
            <threshold>
