...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:pikaf="http://www.philips.com/pikachu/functions/1.0" exclude-result-prefixes="fn pikaf xs">
			...
			<xsl:element name="xsl:template">
				...
				<xsl:element name="xsl:for-each">
					...
					<NavigationValue>
						...
						<xsl:element name="xsl:choose">
							<xsl:element name="xsl:when">
								<xsl:attribute name="test"><xsl:text>MustHaveValue='true' and UseFixedFV='true'</xsl:text></xsl:attribute>
								<xsl:element name="FilterValue">
									<xsl:element name="xsl:value-of">
										<xsl:attribute name="select"><xsl:text>FixedFV</xsl:text></xsl:attribute>
									</xsl:element>
								</xsl:element>
							</xsl:element>
							<xsl:element name="xsl:when">
								<xsl:attribute name="test"><xsl:text>MustHaveValue='true' and UseFixedFV='false'</xsl:text></xsl:attribute>
								<xsl:element name="FilterValue">
									<xsl:element name="xsl:value-of">
										<xsl:attribute name="select"><xsl:text>XPath_DV</xsl:text></xsl:attribute>
									</xsl:element>
  ...
  -->
	<xsl:template match="FilterKey/XPath">
		...
		<xsl:element name="xsl:template">
			...
			<!--
        Create a unique priority. There can be a total of 1000 XPath expressions
        in one FilterGroup, either in FilterKey or FilterValue descendants of the group 
      -->
			<xsl:attribute name="priority" select="-($group-pos * 1000 + $key-pos)"/>
			<!--
  ...
  -->
	<xsl:template match="FilterValue/XPath">
        ...
        in one FilterGroup, either in FilterKey or FilterValue descendants of the group 
      -->
			<xsl:attribute name="priority" select="-($group-pos * 1000 + $key-pos + $value-pos)"/>
			<xsl:element name="xsl:param">
				<xsl:attribute name="name">filters</xsl:attribute>
			</xsl:element>
			<xsl:element name="xsl:variable">
