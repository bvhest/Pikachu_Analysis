      ...
      <xsl:choose>
        <xsl:when test="$reload='true'">        
          ...
          <xsl:copy>
            ...
            <status><xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PMT_Master']/sql:status"/></status>      
          </xsl:copy>      
        </xsl:when>
      <xsl:otherwise>   
        <xsl:copy>
          ...
          <xsl:element name="status">
            <xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PMT_Master']/sql:status"/>    
          </xsl:element>
        </xsl:copy>      
      </xsl:otherwise>             
    </xsl:choose>       
