...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sql="http://apache.org/cocoon/SQL/2.0">
  ...
  <xsl:template match="root">
    <xsl:copy copy-namespaces="no">
      <delta>
        <xsl:apply-templates select="delta/ObjectsMsg" mode="clean"/>
      </delta>
      <cache>
        <xsl:apply-templates select="cache/ObjectsMsg" mode="clean"/>
      </cache>  
      <store>
        <xsl:apply-templates select="delta/ObjectsMsg"/>
      </store>
    </xsl:copy>
  </xsl:template>
  ...
  <xsl:template match="Object" mode="clean">
    <xsl:copy copy-namespaces="no">
