...
xmlns:cmc2-f="http://www.philips.com/cmc2-f" extension-element-prefixes="cmc2-f" >
	...
	<xsl:template match="/">
		<entries ct="{$ct}" l="{catalog-definition[1]/@l}" ts="{$ts}" dir="{$dir}" batchnumber="{$batchnumber}">
			<process/>
			<globalDocs/>
			<xsl:apply-templates select="catalog-definition"/>
		</entries>
	</xsl:template>
	...
	<xsl:template match="catalog-definition">
		...
		<!-- the localisation may be determined by the item being processed - for eg translation inport or is hard coded. In this case it is aways master_global -->
