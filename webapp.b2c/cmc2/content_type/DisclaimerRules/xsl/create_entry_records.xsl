 ...
 xmlns:cmc2-f="http://www.philips.com/cmc2-f" extension-element-prefixes="cmc2-f" >
	...
	<xsl:template match="/">
		<entries ct="{$ct}" l="{$l}" ts="{$ts}" dir="{$dir}" batchnumber="{$batchnumber}">
			<process/>
			<globalDocs/>
			<xsl:apply-templates select="product-disclaimers"/>
		</entries>
	</xsl:template>
	
	<xsl:template match="product-disclaimers">
		...
		<xsl:variable name="objectId" select="'disclaimer'"/>
        ...
        <status><xsl:value-of select="'Loaded'"/></status>
			</octl-attributes>
		</entry>
	</xsl:template>
