...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sql="http://apache.org/cocoon/SQL/2.0" xmlns:cinclude="http://apache.org/cocoon/include/1.0" exclude-result-prefixes="sql xsl cinclude">
  ...
  <!--
    Fix feature codes of Filter features that have product specific feature images.
    These codes are suffixed with the product CTN. 
  -->
  <xsl:template match="entry/content/octl/sql:rowset/sql:row[sql:content_type='PMT_Translated']/sql:data/Product/Filters/Purpose/Features/Feature">
     <xsl:copy copy-namespaces="no">
   ...
   <xsl:template match="entry/content/octl/sql:rowset/sql:row[sql:content_type='PMT_Translated']/sql:data/Product//@operation"/>
</xsl:stylesheet>
