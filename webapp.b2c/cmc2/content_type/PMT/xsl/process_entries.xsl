...
<xsl:stylesheet version="2.0" 
  ...
  <xsl:variable name="currdatetime" select="substring(string(current-dateTime()),1,19)"/>  
	...
	<xsl:template match="@*|node()">
	 ...
	 <xsl:template match="octl|sql:rowset|sql:row[sql:content_type='PMT_Translated']|sql:data">
	 		<xsl:apply-templates select="@*|node()"/>
	 </xsl:template>
	...
	<xsl:template match="octl-attributes[../@valid='true']">
      <xsl:choose>
        <xsl:when test="$reload='true'">        
          <xsl:variable name="current-mlm" select="../currentmasterlastmodified_ts" />
          <xsl:variable name="current-lm" select="../currentlastmodified_ts" />      
          <xsl:copy>
            ...
            <status><xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PMT_Translated']/sql:status"/></status>	  
            <marketingversion><xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PMT_Translated']/sql:marketingversion"/></marketingversion> 
          </xsl:copy>      
        </xsl:when>
      <xsl:otherwise>   
    		<xsl:copy>
    			...
    			<xsl:element name="masterlastmodified_ts">
    		   			<xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PMT_Translated']/sql:masterlastmodified_ts"/>	  
    			</xsl:element>
    			<xsl:element name="status">
    				<xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PMT_Translated']/sql:status"/>	  
    			</xsl:element>
				<xsl:element name="marketingversion">
    				<xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PMT_Translated']/sql:marketingversion"/>	  
    			</xsl:element>
    		</xsl:copy>
      </xsl:otherwise>             
    </xsl:choose>        
