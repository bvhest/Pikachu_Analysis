  ...
  <xsl:variable name="timestampnumeric" select="replace(replace(replace($timestamp,':',''),'-',''),'T','')"/>
	...
	<xsl:template match="/">
		<source:write xmlns:source="http://apache.org/cocoon/source/1.0">
			<source:source><xsl:value-of select="concat($dir,'/logs/ErrorReport_',$timestampnumeric,'.xml')"/></source:source>
			<source:fragment>
				<report runTimestamp="{$timestamp}">
          <error>
            <message>Too many changed products</message>
            <threshold><xsl:value-of select="$threshold"/></threshold>
            <filecount><xsl:value-of select="$filecount"/></filecount>
          </error>            
				</report>
			</source:fragment>
		</source:write>
