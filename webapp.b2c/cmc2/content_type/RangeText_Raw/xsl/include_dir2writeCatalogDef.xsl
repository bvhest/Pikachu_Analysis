...
<xsl:stylesheet version="2.0"
   ...
   <xsl:template match="dir:directory">
      ...
      <xsl:analyze-string select="$timestamp" regex="^(....)(..)(..)(..)(..)(..)$">
		...
		<source:write xmlns:source="http://apache.org/cocoon/source/1.0">
			<source:source>
				<xsl:value-of select="concat($dir,$prefix,'_',$timestamp,'_def.xml')"/>
			</source:source>
			<source:fragment>
				<catalog-definition o="{$prefix}" ct="catalog_definition" l="none" DocTimeStamp="{$dts/value}">
   				 	<xsl:apply-templates/>
				</catalog-definition>
			</source:fragment>
		</source:write>
  ...
  <xsl:template match="dir:file">
    ...
    <cinclude:include><xsl:attribute name="src" select="concat('cocoon:/writeCatalogDef/',$ct,'/',$timestamp,'/',$filestem,'.',$fileextn)"/></cinclude:include>
  </xsl:template>
</xsl:stylesheet>