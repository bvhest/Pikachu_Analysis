    ...
    <xsl:copy copy-namespaces="no">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:apply-templates select="@*|node()"/>
          <email:sendmail>
            <email:subject>PMT_Raw: restore inbox failed</email:subject>
            <email:body mime-type="text/plain">
              <xsl:text>The inbox could not be sucessfuly restored.
It is not safe to rerun the process with an override.

Total number of files: </xsl:text>
