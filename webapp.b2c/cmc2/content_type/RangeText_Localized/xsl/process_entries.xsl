  ...
  <xsl:template match="octl-attributes[../@valid='true']">
    <xsl:choose>
      <xsl:when test="$reload='true'">        
        ...
        <xsl:copy>
          <xsl:apply-templates select="@*|node()[not(local-name()='masterlastmodified_ts' or local-name()='lastmodified_ts' or local-name()='status')]"/>
          ...
          <status><xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='RangeText_Raw']/sql:status"/></status>      
        </xsl:copy>      
      </xsl:when>
      <xsl:otherwise> 
        <xsl:copy>
          <xsl:apply-templates select="@*|node()[not(local-name()='masterlastmodified_ts' or local-name()='status' or  local-name()='islocalized')]"/>
          ...
          <xsl:element name="islocalized">
            <xsl:value-of select="if (upper-case(../content/octl/sql:rowset/sql:row/sql:data/Node/@IsLocalized)='TRUE') then 1 else 0"/>
          </xsl:element>
        </xsl:copy>
      </xsl:otherwise>             
    </xsl:choose>           
