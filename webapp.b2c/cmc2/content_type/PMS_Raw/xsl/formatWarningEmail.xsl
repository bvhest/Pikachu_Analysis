...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="source">
   <xsl:copy>
      <warning>Problems exist with PMS_Raw xml feed</warning>
      <email:sendmail>
        <email:from><xsl:value-of select="$sender"/></email:from>
        <email:to><xsl:value-of select="$recipient"/></email:to>
        <email:subject><xsl:value-of select="concat('Warning: Problems with PMS_Raw xml feed  (instance:',substring-before(substring-after($server,'http://'),':'),'). Autogenerated email sent at ',$timestamp,' by Pika Chu')"/></email:subject>
        <email:body mime-type="text/plain">
Too many changed products
=========================
<xsl:text>
