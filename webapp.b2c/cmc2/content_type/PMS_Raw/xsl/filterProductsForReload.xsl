...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sql="http://apache.org/cocoon/SQL/2.0">
  ...
  <xsl:template match="entry">
    <entry>
      ...
      <!--
         | Process only those products whose lastmodified timestamp matches the masterlastmodified_ts timestamp
         | in the OCTL table.
         -->
      <xsl:choose>
        <xsl:when test="not ($current-mlm) ">
          ...
          <incoming-lm>
            <xsl:value-of select="$incoming-lm" />
          </incoming-lm>
          <current-mlm>
        ...
        <xsl:when test="$incoming-lm le $current-mlm">
          <!-- Process: timestamp is less than or equal to current, but content should be fixed -->
          ...
          <octl-attributes>
            <xsl:copy-of copy-namespaces="no"
              select="octl-attributes/node()[not(local-name() = 'lastmodified_ts' or local-name() = 'masterlastmodified_ts')]" />
            <lastmodified_ts>
            ...
            <remark>
              <xsl:value-of select="concat('RELOADED from source XML on ',$currdatetime)" />
            </remark>
          </octl-attributes>
