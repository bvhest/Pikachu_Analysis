...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sql="http://apache.org/cocoon/SQL/2.0"
 xmlns:cmc2-f="http://www.philips.com/cmc2-f" extension-element-prefixes="cmc2-f" >
  ...
  <xsl:template match="Product">
    ...
    <xsl:variable name="objectId" select="MasterData/CTN"/>
    <xsl:variable name="MarketingVersion" select="MasterData/PMTVersion"/>
    <xsl:variable name="status" select="MasterData/ProductStatus"/>
    <xsl:variable name="remark" select="MasterData/PMTEditCopy/Status"/>
    <entry o="{$objectId}"  ct="{$ct}" l="{$l}"  valid="true">
      ...
      <octl-attributes>
        ...
        <!-- store edit status (InEdit, InSigning, Rejected) in remark -->
