...
<xsl:stylesheet version="2.0" 
  ...
  <!-- Ala_test expert review removed -->
  <xsl:template match="Award[@AwardType=('ala_expert')] "/>
      ...
      <xsl:choose>
        <xsl:when test="$reload='true'">        
          ...
          <xsl:copy>
            ...
            <status><xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PMT_Raw']/sql:status"/></status>
            <marketingversion><xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PMT_Raw']/sql:marketingversion"/></marketingversion>      
          </xsl:copy>      
        </xsl:when>
      <xsl:otherwise>   
        <xsl:copy>
          ...
          <xsl:element name="status">
            <xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PMT_Raw']/sql:status"/>    
          </xsl:element>
          <xsl:element name="marketingversion">
            <xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PMT_Raw']/sql:marketingversion"/>    
          </xsl:element>
        </xsl:copy>      
      </xsl:otherwise>             
    </xsl:choose>       
