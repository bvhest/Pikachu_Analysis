      ...
      <xsl:choose>
        <xsl:when test="$reload='true'">        
          ...
          <xsl:copy>
            ...
            <status><xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PCT_Translated']/sql:status"/></status>
            <marketingversion><xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PCT_Translated']/sql:marketingversion"/></marketingversion>      
          </xsl:copy>      
        </xsl:when>
      <xsl:otherwise>   
        <xsl:copy>
          ...
          <xsl:element name="status">
            <xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PCT_Translated']/sql:status"/>    
          </xsl:element>
          <xsl:element name="marketingversion">
            <xsl:value-of select="../content/octl/sql:rowset/sql:row[sql:content_type='PCT_Translated']/sql:marketingversion"/>    
          </xsl:element>
        </xsl:copy>      
      </xsl:otherwise>             
    </xsl:choose>       
