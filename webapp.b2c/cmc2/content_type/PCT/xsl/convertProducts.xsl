...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sql="http://apache.org/cocoon/SQL/2.0" xmlns:i="http://apache.org/cocoon/include/1.0" exclude-result-prefixes="sql xsl i">
	...
	<!--  -->
        ...
        <xsl:variable name="resourceTypes" select="$doctypesfile/doctypes/doctype[@pct='yes']/@code"/>
 
	<xsl:template match="/">
		<xsl:apply-templates select="@*|node()"/>
	</xsl:template>
	<!--  -->
	<xsl:template match="octl[sql:rowset/sql:row/sql:content_type='PMT']"/>
	...
	<xsl:template match="Product" exclude-result-prefixes="i sql xsl">
		<Product>
			...
			<xsl:apply-templates select="../../../../../octl/sql:rowset/sql:row[sql:content_type='PMT']/sql:data/Product/MarketingVersion"/>
			<xsl:apply-templates select="../../../../../octl/sql:rowset/sql:row[sql:content_type='PMT']/sql:data/Product/MarketingStatus"/>
			
			<xsl:choose>
				<xsl:when test="../../../../../octl/sql:rowset/sql:row[sql:content_type='PMT']/sql:data/Product/NamingString">
					<xsl:apply-templates select="../../../../../octl/sql:rowset/sql:row[sql:content_type='PMT']/sql:data/Product/NamingString"/>
				</xsl:when>
				<xsl:otherwise>
...
<!-- stripped feed for first phase -->
				<xsl:choose>
					<xsl:when test="substring($locale,1,2)='en' or $productIsLocalized">
						<xsl:apply-templates select="../../../../../octl/sql:rowset/sql:row[sql:content_type='AssetList'][sql:localisation=$locale]/sql:data/object/Asset[ResourceType=$resourceTypes]"/>
						<xsl:apply-templates select="../../../../../octl/sql:rowset/sql:row[sql:content_type='AssetList'][sql:localisation='master_global']/sql:data/Product/AssetList/Asset[not(Language='en_US' and ResourceType='PSS')] [ResourceType=$resourceTypes] "/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:apply-templates select="../../../../../octl/sql:rowset/sql:row[sql:content_type='AssetList']/sql:data/object/Asset[ResourceType=$resourceTypes]"/>
					</xsl:otherwise>
				</xsl:choose>
<!-- sf-->
