...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sql="http://apache.org/cocoon/SQL/2.0" xmlns:cinclude="http://apache.org/cocoon/include/1.0" exclude-result-prefixes="sql xsl cinclude">
 ...
 <xsl:template match="@*|node()" mode="objectAsset">
    <xsl:copy copy-namespaces="no">
      <xsl:apply-templates select="@*|node()"  mode="objectAsset"/>
    </xsl:copy>
  </xsl:template>
  <!--  -->
  ...
  <xsl:template match="PublicResourceIdentifier" mode="objectAsset">
  <xsl:variable name="ctn" select="../../../../../../../../../../../../../../@o"/>
  <xsl:variable name="scy" select="substring(../../../../../../../../../../../../../../@l,4,2)"/>
    <xsl:copy copy-namespaces="no">
		<xsl:value-of select="concat(.,'&amp;scy=',$scy,'&amp;ctn=',$ctn)"/>  
    </xsl:copy>
  </xsl:template>      
    ...
    <xsl:for-each-group select="octl/sql:rowset/sql:row" group-by="sql:object_id">
      <Object>       
        ...
        <xsl:for-each select="current-group()/sql:data/object/Asset">
          ...
          <xsl:apply-templates select="." mode="objectAsset"/>
        </xsl:for-each>
      </Object>
    </xsl:for-each-group>
    </xsl:copy>
