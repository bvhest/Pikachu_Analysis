...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sql="http://apache.org/cocoon/SQL/2.0"
  xmlns:saxon="http://saxon.sf.net/" exclude-result-prefixes="sql xsl">
  ...
  <xsl:template match="entries">
    <entries>
      ...
      <xsl:attribute name="type"><xsl:value-of select="'xUCDM-product'" /></xsl:attribute>
  ...
  <xsl:template match="node()|@*" mode="Product">
    ...
    <!-- 
      Check if current node is translated attribute by retrieving a translated attribute 
      from the translated attributes list for which the name is equal to the current node name.
      Only return the first match. VersionElementName is listed three times in the 
      translated attribute list for instance. We only want one result
    -->
    ...
    <xsl:choose>
      ...
      <xsl:when test="(count($translatedAttribute) > 0 and string() != '') 
                       and not($nodeName = 'WOW' and substring-after(ancestor::entries/@routingCode,'/') = ('CA00324_CA','CA00325_CA'))">
        <!-- 
          This is a translated element.
          Replace the current element by the newly created element enriched 
          with translation meta attributes and process the child elements.
        -->
        <xsl:copy copy-namespaces="no">
          <!-- Generate a key attribute. The value consists of a key and the corresponding code 
             | which can be obtained by evaluating the xpath expression in the codeXPath attribute.
             -->
          <xsl:attribute name="key">
            <xsl:value-of select="$translatedAttribute/@key" />
            <xsl:text>_</xsl:text>
            <xsl:value-of select="saxon:evaluate($translatedAttribute/@codeXPath)" />
          </xsl:attribute>
          <!-- Now copy all the meta attributes -->
          <xsl:copy-of copy-namespaces="no" select="$translatedAttribute/metaAttributes/@*" />
          <!-- Now copy all attributes on the node -->
