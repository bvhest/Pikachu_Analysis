	...
	<xsl:template name="objectAssetList">
		...
		<xsl:for-each-group select="$prd/ObjectAssetList/Object[id=$objectId]/Asset[replace(Language,'-','_') = $locale]" group-by="ResourceType">
			...
			<xsl:sort	select="Language" order="ascending"/>
				...
				<add-item item-descriptor="asset" id="{concat($objectName,'-',$objectId,'-',current-grouping-key(),'-',$localisation,'-',$suffixId)}" repository="/atg/commerce/catalog/ProductCatalog">
							...
							<xsl:choose>
								<xsl:when test="$doctypesfile/doctypes/doctype[@ATGAssets='yes'and @code=current-grouping-key()]">
									<set-property name="internalUrl">
										<xsl:value-of select="concat('/catalog/',$objectId, '-' ,current-grouping-key(),'-',$localisation,'-',$suffixId,'.',$ext  )"/>
									</set-property>
								</xsl:when>
								<xsl:otherwise>
			...
			<add-item repository="/atg/commerce/catalog/ProductCatalog" item-descriptor="assetList" id="{concat($objectName,'-',$objectId,'-',current-grouping-key(),'-',$localisation )}">
