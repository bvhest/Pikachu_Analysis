...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:my="http://www.philips.com/pika"
  ...
  <xsl:template match="/">
    <gsa-template>
      <import-items>
        <xsl:for-each-group
            select="/Products/Product/NamingString/MasterBrand|/Products/Product/NamingString/Partner/PartnerBrand"
            group-by="BrandCode">
          <xsl:for-each select="current-group()">
            ...
            <xsl:if test="position() = 1 and $lastModified &gt; $lastExportDate">
              ...
              <add-item repository="/philips/store/catalog/ProductContentRepository" item-descriptor="brand" id="{$itemCode}">
