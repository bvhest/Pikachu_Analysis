...
-->
<!-- CVS $Id: exception2html_write.xsl,v 1.1 2006/11/29 09:25:57 gwisselink Exp $ -->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ex="http://apache.org/cocoon/exception/1.0" xmlns:source="http://apache.org/cocoon/source/1.0">
	...
	<xsl:param name="dir"/>
	<xsl:variable name="importdate">
		<xsl:value-of select="format-dateTime(current-dateTime(),'[Y,4][M,2][D,2][H,2][m,2][s,2]')"/>
	</xsl:variable>
	<xsl:template match="ex:exception-report">
		<root>
			<xsl:variable name="uri"><xsl:value-of select="ex:cocoon-stacktrace/ex:exception[last()]/ex:locations/ex:location[1]/@uri"/></xsl:variable>
			<xsl:variable name="uriPart"><xsl:value-of select="substring-before(substring-after($uri,'pipes/'),'/')"/></xsl:variable>
			<source:write xmlns:source="http://apache.org/cocoon/source/1.0">
				<source:source>
					<xsl:value-of select="$dir"/>
					<xsl:value-of select="$uriPart"/>
					<xsl:text>/logs/error_</xsl:text>
          ...
          td { padding: 0.1em; }
						...
						<body>
							<xsl:attribute name="onload"><xsl:if test="ex:stacktrace">toggle('stacktrace');</xsl:if><xsl:if test="ex:full-stacktrace">toggle('full-stacktrace');</xsl:if></xsl:attribute>
							<xsl:attribute name="contentID">content</xsl:attribute>
							<h1>
								<xsl:value-of select="$pageTitle"/>
							</h1>
							<p class="message">
								...
								<xsl:for-each select="ex:cocoon-stacktrace/ex:exception">
									...
									<table>
										<xsl:for-each select="ex:locations/*[string(.) != '[cause location]']">
											<!-- [cause location] indicates location of a cause, which  the exception generator outputs separately -->
