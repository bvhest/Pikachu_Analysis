...
<xsl:stylesheet version="2.0" 
  ...
  <!--+
      |
      |  Generates xml for xUCDM_product_external_1_2 (see
      ...
      |
      |  BHE (1/10/2009): - changed order of elements Productname, ProductReferences
      |                   - changed name of ProductReferences to ProductRefs
      +-->
  ...
  <xsl:template match="Products">
    <Products>
      ...
      <xsl:attribute name="DocVersion"><xsl:text>xUCDM_product_external_1_2.xsd</xsl:text></xsl:attribute> 
      <xsl:apply-templates select="node()"/>
    </Products>
  </xsl:template>
  ...
  <xsl:template match="Product">
    <Product>
      ...
      <xsl:call-template name="doAwards">
        <xsl:with-param name="Awards" select="Award[not(@AwardType=('ala_semantic','ala_user','ala_bv_summary' ))]"/>
      </xsl:call-template>
      <!-- end Green2 -->
      <ProductClusters>
...
<!-- BHE: element Range must be omitted from NamingString
...
-->
  ...
  <xsl:template name="doProductReference">
    <!-- ProductReference exists in PCT content -->
  ...
  <xsl:template name="doKeyValuePairs">
    <!-- PMTs may still have separate KeyValuePair elements without a proper container -->
