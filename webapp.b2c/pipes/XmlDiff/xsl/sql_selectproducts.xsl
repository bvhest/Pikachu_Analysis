			...
			<sql:execute-query xmlns:sql="http://apache.org/cocoon/SQL/2.0">
				<sql:query>
						insert into CUSTOMER_LOCALE_EXPORT(CUSTOMER_ID, LOCALE, CTN, FLAG)
							...
							inner join (select * from TRIGO_LOCALIZED_PRODUCTS where locale = '<xsl:value-of select="$locale"/>') TLP
							on LP.ID = TLP.ID
							and LP.LOCALE = TLP.LOCALE
					...
					update CUSTOMER_LOCALE_EXPORT cle
	                    ...
	                    and (lasttransmit is null 
	                         or lasttransmit &lt;   (select lp.lastmodified 
													 ...
													 inner join TRIGO_LOCALIZED_PRODUCTS tlp 
													   on  lp.id = tlp.id 
													   and lp.locale = tlp.locale 
