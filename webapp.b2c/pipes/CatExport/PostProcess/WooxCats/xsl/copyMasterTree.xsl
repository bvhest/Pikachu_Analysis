...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- Fix the country part of the philips:id -->
   <xsl:template match="philips:id">
    ...
    <xsl:copy copy-namespaces="no">
      <xsl:value-of select="replace(., '_GLOBAL_WOOX', concat('_', substring($locale, 4), '_WOOX'))" />
       <xsl:value-of select="replace(., '_GLOBAL_WOOX', concat('_', $locale, '_WOOX'))" /> 
    </xsl:copy>
  </xsl:template> 
  ...
  <xsl:template match="philips:locale">
    ...
    <xsl:copy copy-namespaces="no">
      ...
      <xsl:value-of select="concat('_', $locale)" />
    </xsl:copy>
  </xsl:template>
