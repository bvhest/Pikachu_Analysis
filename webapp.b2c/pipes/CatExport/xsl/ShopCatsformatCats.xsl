    ...
    <xsl:for-each-group select="sql:row" group-by="sql:catalogcode">
      ...
      <xsl:for-each-group select="current-group()" group-by="sql:groupcode">
        <philips:category>
          ...
          <philips:image/>
          <philips:thumbNailImage/>
          <philips:imageHover/>
          <philips:externalUrl/>
          <xsl:choose>
            <xsl:when test="$locale='MASTER'">
              <philips:seoName romanize="true" locale="en_US">
                <xsl:choose>
          ...
          <philips:uniqueCatName>
            <xsl:value-of select="lower-case(substring(sql:groupcode, 1, string-length(sql:groupcode)-3))" />
          </philips:uniqueCatName>
        </philips:category>
      </xsl:for-each-group>
      ...
      <xsl:for-each-group select="current-group()" group-by="concat(sql:categorycode,sql:groupcode)">
        <philips:category>
          ...
          <philips:uniqueCatName>
            <xsl:value-of select="lower-case(substring(sql:categorycode, 1, string-length(sql:categorycode)-3))" />
          </philips:uniqueCatName>
        </philips:category>
      </xsl:for-each-group>
      ...
      <xsl:for-each-group select="current-group()" group-by="sql:subcategorycode">
        <xsl:for-each select="current-group()">
          <philips:category>
            ...
            <philips:uniqueCatName>
              <xsl:value-of select="lower-case(substring(sql:subcategorycode, 1, string-length(sql:subcategorycode)-3))" />
            </philips:uniqueCatName>
          </philips:category>
        </xsl:for-each>
      </xsl:for-each-group>
