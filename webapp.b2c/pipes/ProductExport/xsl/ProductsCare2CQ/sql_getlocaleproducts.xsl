...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sql="http://apache.org/cocoon/SQL/2.0" exclude-result-prefixes="sql xsl">
  ...
  <xsl:template match="/">
    <!-- default = PMT, otherwise select from sourceCT variabele.
     +-->
    <xsl:variable name="source" select="if ($sourceCT != '') then $sourceCT else 'PCT'" />
    ...
    select
        ...
        <sql:execute-query>          
          <sql:query name="careAssetList">                  
									SELECT AL.OBJECT_ID
									     , AL.DOCTYPE
									     , AL.LOCALE
									     , AL.PUBLICRESOURCEIDENTIFIER
									     , TO_CHAR(AL.LASTMODIFIED,'yyyy-mm-dd') LASTMODIFIED 
									  FROM ASSET_LISTS AL
									  WHERE AL.OBJECT_ID = '<sql:ancestor-value name="id" level="1"/>'
									    AND (( AL.DOCTYPE IN ('PSS','DFU','QSG') and AL.LOCALE NOT LIKE 'en%') or 
									         ( AL.DOCTYPE IN ('PSS','DFU','QSG') and AL.LOCALE = 'en_GB' ) or 
									         ( AL.DOCTYPE IN ('DFU','QSG')       and AL.LOCALE = 'en_US' ))  
									    AND AL.DELETED = 0
          </sql:query>
        </sql:execute-query>
        
         <sql:execute-query>          
          <sql:query name="careSoftAssetList">       
						        SELECT AL.OBJECT_ID
										     , AL.DOCTYPE
										     , AL.LOCALE
										     , AL.PUBLICRESOURCEIDENTIFIER
										...
										WHERE AL.OBJECT_ID = '<sql:ancestor-value name="id" level="1"/>'
												 ...
												 AND AL.DELETED = 0
												 AND AL.DOCTYPE IN ('BUS','CAS','CCU','CDI','CIU','CMS','D2K','D95',
												                    'D98','D9S','DLX','DM9','DMD','DME','DMX','DNT',
												                    'DTS','DV6','DW7','DW8','DWV','DXP','DXX','FCC',
												                    'FUD','FUM','FUS','FUZ','FWF','HF1','HF3','HF4',
												                    'ICD','ICS','L2S','L3S','LDS','MCS','MJS','MMS',
												                    'MUS','PAL','PAS','PCS','PES','PIL','PMM','PMS',
												                    'PMW','PMX','PSB','PVT','PXP','QUS','S2S','S4S',
												                    'SCS','SW7','TUS','TVS')
          </sql:query>
        </sql:execute-query>
       </sql:execute-query>
