...
<xsl:stylesheet version="2.0" xmlns="http://www.bazaarvoice.com/xs/PRR/ProductFeed/5.6" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sql="http://apache.org/cocoon/SQL/2.0" xmlns:cmc2-f="http://www.philips.com/cmc2-f" xmlns:asset-f="http://www.philips.com/xucdm/functions/assets/1.2" extension-element-prefixes="sql cmc2-f asset-f ">
	...
	<xsl:template match="Product" mode="product">
		...
		<xsl:variable name="family-name" select="../../sql:rowset[@name='family']/sql:row[1]/sql:subcategoryname"/>
      ...
      <xsl:choose>
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
			...
			<!-- CJ : C1014188 - Product URL change for Saeco-->
			
			<xsl:variable name="saeco-family-name">
				<xsl:choose>
					<xsl:when test="$family-name != ''">
						<xsl:value-of select="$family-name"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="$Cat-Row/sql:categoryname"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			
			<xsl:element name="ProductPageUrl">
				<xsl:value-of select="asset-f:buildProductDetailPageUrl-saeco(., $locale, $system, $saeco-family-name, $domains )"/>
			</xsl:element>
