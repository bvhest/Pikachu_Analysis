...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sql="http://apache.org/cocoon/SQL/2.0" exclude-result-prefixes="sql xsl">
  ...
  <xsl:template match="/">
    ...
    select
        ...
        sub1.localisation,
        sub1.product_type,
        sub1.catalogtype as catalogtype,
        sub1.local_going_price,
        sub1.catalogtype as categorization_catalogtype,
        o.data
        <xsl:choose> 
        ...
        from (
            ...
            where co.CUSTOMER_ID = cc.catalog_type
            group by o.object_id, ll.language, o.status, o.localisation, cc.product_type
          ) sub1
          inner join octl o
