      ...
      <sql:execute-query>
        <sql:query name="Cat-tree">
          select cle.ctn ID
               ...
               , '<xsl:value-of select="concat('en_', $country)"/>' as master_locale
               , c.groupcode, c.groupname
               , c.categorycode, c.categoryname
               , c.subcategorycode, c.subcategoryname
               
          FROM CUSTOMER_LOCALE_EXPORT cle 

          INNER JOIN categorization c 
             ON c.catalogcode = cle.ctn
          ...
          order by c.groupcode, c.categorycode, c.subcategorycode
        </sql:query>
        </sql:execute-query>
      ...
      <sql:execute-query>
        <sql:query name="Product">
          SELECT sub.id, sub.masterlastmodified, sub.lastmodified, sub.timestamp
               , sub.lastexportdate, sub.catalog_type
