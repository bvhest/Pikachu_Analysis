...
<xsl:stylesheet version="2.0"
  ...
  <xsl:import href="../../common/xsl/xucdm-product-external-cq.xsl"/>
  ...
  <xsl:template match="sql:id|sql:language|sql:sop|sql:eop|sql:sos|sql:eos|sql:local_going_price|sql:categorization_catalogtype|sql:priority|sql:content_type|sql:localisation|sql:catalogtype"/>
    ...
    <Assets>
      <xsl:choose>
        <xsl:when test="$type = 'locale'">
          ...
          <!-- Changing  $catalogtype into 'consumer' as per FS comment  -->
          <xsl:sequence select="asset-f:createVirtualAsset($id,'URL',$locale, asset-f:buildProductDetailPageUrl(., $locale, $system, 'consumer', $domains), $lastModified, 'Product URL', '', '')"/>
  ...
  <!-- modify the GreenData-element, remove some attributes and elements (conditionally)
  -->
  <xsl:template match="@publish[ancestor::GreenData]"/>
  <xsl:template match="@isGlobal[ancestor::GreenData]"/>
  <xsl:template match="GreenChapter/@MeetRegulations"/>
  
  <!-- rename BlueAngel to TheBlueAngel -->
  <xsl:template match="BlueAngel">
    <xsl:element name="TheBlueAngel">
      <xsl:apply-templates select="node()|@*"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="*[name()=('ApplicableFor','Code','LongDescription','Name','ShortDescription','Text') and parent::*/@publish='false']"/>
  <xsl:template match="*[name()=('ApplicableFor','Code','LongDescription','Name','ShortDescription','Text') and parent::PhilipsGreenLogo/@isGreenProduct='false']"/>
  <xsl:template match="*[name()=('ApplicableFor','Code','LongDescription','Name','ShortDescription','Text') and parent::BlueAngel/@isBlueAngelProduct='false']"/>
  <xsl:template match="*[name()=('ApplicableFor','Code','LongDescription','Name','ShortDescription','Text') and parent::EcoFlower/@isEcoFlowerProduct='false']"/>
