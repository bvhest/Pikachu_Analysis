...
<xsl:stylesheet version="2.0" 
  ...
  <xsl:template name="doAssets">
    ...
    <xsl:variable name="description" select="$doctypes/doctype[@code = $doctype]/@description"/><!-- ETL-->
    <Assets>
    <Asset code="{$escid}" type="SPD" locale="{$locale}" number="001" description="Power Distribution" extension="pdf"><xsl:value-of select="concat($nonimagepath,$id,'&amp;doctype=SPD&amp;laco=',$language)"/></Asset>
    <Asset code="{$escid}" type="DMC" locale="{$locale}" number="001" description="Dimmer Compatibility" extension="pdf"><xsl:value-of select="concat($nonimagepath,$id,'&amp;doctype=DMC&amp;laco=',$language)"/></Asset> -->
