      ...
      <sql:query>
        insert into customer_locale_export(customer_id, locale, ctn, flag)
          ...
          where cle.CUSTOMER_ID is NULL
            ...
            and o.content_type in ('PMT_Master')
                and (
         ...
         where locale = 'MASTER'
           ...
           and ctn in (
            ...
            <!--  inner join mv_cat_obj co
                 on co.object_id = o.object_id
                on (ch.catalog = 'ALL' or co.catalog=cc.catalog_type) -->

		inner join catalog_objects co
             --on co.object_id = o.object_id
            on (co.catalog_id='ProductMasterDataCatalog') 
              ...
              where ch.name='<xsl:value-of select="$channel"/>'
               ...
               <!-- and ((sop - ch.PUBLICATIONOFFSET_SOP &lt; sysdate
                ...
                or (sos - ch.PUBLICATIONOFFSET_SOP &lt; sysdate
                and eos + ch.PUBLICATIONOFFSET_EOP &gt; sysdate)) -->
		and o.content_type in ('PMT_Master')
                and (
                      (o.content_type = 'PMT_Master' and o.status = 'Final Published') 
                )
