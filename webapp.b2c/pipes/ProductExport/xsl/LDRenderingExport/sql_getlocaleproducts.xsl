...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sql="http://apache.org/cocoon/SQL/2.0" exclude-result-prefixes="sql xsl">
  ...
  <xsl:template match="/">
    ...
    left outer join octl ld
        on ld.content_type = 'LogisticData'
       and ld.localisation = 'master_'||substr(sub1.localisation,4,2)
       and ld.object_id    = sub1.object_id
       and ld.status       = 'Final Published' 
          ...
          <sql:query name="cat">
              ...
              inner join vw_object_categorization oc 
                   ...
                   where oc.object_id   = '<sql:ancestor-value name="id" level="1"/>'
                     and oc.catalogcode = '<sql:ancestor-value name="catalogtype" level="1"/>'
                     and rownum &lt; 2
          </sql:query>
        </sql:execute-query>
       </sql:execute-query>
