	...
	<xsl:template match="/">
        ...
        <xsl:for-each-group select="/Products/Product" group-by="@Catalog">
          <xsl:variable name="currentcatalog" select="current-grouping-key()"/>
  				<xsl:for-each-group select="/Products/Product[@Catalog=$currentcatalog]/PartnerLogo" group-by="PartnerLogoCode">
  					<xsl:for-each select="current-group()">
  						...
  						<xsl:variable name="lastExportDate"     select="../@LastExportDate"     as="xsl:dateTime"/>
  							...
  							<add-item repository="/atg/commerce/catalog/ProductCatalog" item-descriptor="partner-logo" id="{$itemCode}">
