...
<xsl:stylesheet version="2.0" 
  ...
  <!--xsl:template match="RichText[@type='ProofPoint']" mode="copyRichtext"/-->
  <!--xsl:template match="RichText[@type='proofPoint']" mode="copyRichtext"/-->
  
   <xsl:template match="@*|node()" mode="copyRichtext">
    <xsl:copy copy-namespaces="no">
      <xsl:apply-templates select="@*|node()" mode="copyRichtext"/>
    </xsl:copy>
  </xsl:template>
  ...
  <xsl:template name="product-translation">
    ...
    <add-item repository="/atg/commerce/catalog/ProductCatalog" item-descriptor="product-translation" id="{concat($atgCTN,'_',$catalogLocale)}">
              <xsl:apply-templates select="RichTexts" mode="copyRichtext"/>
    </add-item>
  </xsl:template>
