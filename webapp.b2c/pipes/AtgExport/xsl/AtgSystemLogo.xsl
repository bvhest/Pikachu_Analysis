	...
	<xsl:template match="/">
        ...
        <xsl:for-each-group select="/Products/Product" group-by="@Catalog">
          <xsl:variable name="currentcatalog" select="current-grouping-key()"/>
  				<xsl:for-each-group select="/Products/Product[@Catalog=$currentcatalog]/SystemLogo" group-by="SystemLogoCode">
  					<xsl:for-each select="current-group()">
  						...
  						<xsl:if test="position() = 1 and $lastModified &gt; $lastExportDate">
  							...
  							<add-item repository="/atg/commerce/catalog/ProductCatalog" item-descriptor="system-logo" id="{$itemCode}">
