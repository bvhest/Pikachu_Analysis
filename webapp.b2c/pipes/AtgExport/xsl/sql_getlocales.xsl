  ...
  <xsl:template match="/">
    <root>
      <sql:execute-query xmlns:sql="http://apache.org/cocoon/SQL/2.0">
        ...
        <!-- Retrieve all locales that are exportable for this channel. -->
          select distinct 
                  ll.locale
                , ll.islatin
                , max(case
                        when cc.masterlocaleenabled>0 then 1
                        else 0
                        end
                     ) as masterlocaleenabled
      ...
      <sql:execute-query>
        <sql:query name="timestamp">
          <xsl:choose>
            ...
            <xsl:otherwise>
              select startexec from channels where name = '<xsl:value-of select="$channel" />FullExtract'
            </xsl:otherwise>
          </xsl:choose>
        </sql:query>
      </sql:execute-query>
