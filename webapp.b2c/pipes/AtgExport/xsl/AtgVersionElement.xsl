	...
	<xsl:template match="/">
        ...
        <xsl:for-each-group select="/Products/Product" group-by="@Catalog">
          <xsl:variable name="currentcatalog" select="current-grouping-key()"/>
  				<xsl:for-each-group select="/Products/Product[@Catalog=$currentcatalog]/NamingString/node()/VersionElementCode" group-by=".">
  					<xsl:for-each select="current-group()">
  						...
  						<xsl:if test="position() = 1 and $lastModified &gt; $lastExportDate">
  							...
  							<xsl:variable name="itemCode" select="concat(.,'_',$catalogLocale)"/>
  							<add-item repository="/atg/commerce/catalog/ProductCatalog" item-descriptor="version-element-translation" id="{$itemCode}">
  								<set-property name="name">
  									<xsl:value-of select="../VersionElementName"/>
  								</set-property>
  							</add-item>
  							<add-item repository="/atg/commerce/catalog/ProductCatalog" item-descriptor="version-element" id="{.}">
