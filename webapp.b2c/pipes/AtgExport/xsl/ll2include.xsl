...
<xsl:stylesheet version="2.0"
	...
	<xsl:param name="exportmasterlocale"/>
			...
			<xsl:when test="$delta='y'">
				<xsl:choose>
					<xsl:when test="number($count) > 0">
						<cinclude:include src="{concat('cocoon:/exportSub.',$timestamp,'.',$country,'.',$locale,'.',sql:row[1]/sql:maxbatch,'.',sql:row[1]/sql:maxbatch)}"/>
						<xsl:if test="$exportmasterlocale='1'">
							<cinclude:include src="{concat('cocoon:/exportSubMaster.',$timestamp,'.',$country,'.',$locale,'.',sql:row[1]/sql:maxbatch,'.',sql:row[1]/sql:maxbatch)}"/>
						</xsl:if>
					</xsl:when>
					<xsl:otherwise>
	...
	<xsl:template match="sql:row">
		...
		<cinclude:include src="{concat('cocoon:/exportSub.',$runtimestamp,'.',$country,'.',$locale,'.',$batch,'.',$maxbatch)}"/>
		<xsl:if test="$exportmasterlocale='1'">
			<cinclude:include src="{concat('cocoon:/exportSubMaster.',$timestamp,'.',$country,'.',$locale,'.',$batch,'.',$maxbatch)}"/>
		</xsl:if>
	</xsl:template>
