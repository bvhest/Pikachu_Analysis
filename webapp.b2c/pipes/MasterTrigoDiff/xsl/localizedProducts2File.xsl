...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:source="http://apache.org/cocoon/source/1.0" xmlns:sql="http://apache.org/cocoon/SQL/2.0">
	...
	<!-- -->
	<xsl:variable name="v_table" select="if($table eq 'LP') then 'TRIGO_LOCALIZED_PRODUCTS' else if($table eq 'TLP') then 'MASTER_PRODUCTS' else ()"/>		
						...
						<sql:query>
							<xsl:choose>
								<xsl:when test="$join eq 'INNER'">						
									...
									from TRIGO_LOCALIZED_PRODUCTS LP INNER JOIN MASTER_PRODUCTS TLP on LP.ID = TLP.ID
