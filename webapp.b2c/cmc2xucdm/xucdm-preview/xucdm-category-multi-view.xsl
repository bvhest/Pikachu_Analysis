...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.0">
  <!-- -->
  <xsl:import href="xucdm-category-generic.xsl"/>
  <!-- -->
    ...
    <html>
      ...
      <body contentID="content">
        <div class="philips-body">
          <table border="1">
            <thead>
              <xsl:call-template name="cat_ErrorTable"/>
            </thead>
            <tbody>
  ...
  <xsl:template match="//Catalog" mode="cat">
    ...
    <tr>
      <td valign="top" align="left" width="150px">
        <b>Catalogs</b>
        <br/>
        <xsl:for-each select="//Catalog/CatalogName">
          <xsl:choose>
            <xsl:when test="text()=$CatalogName">
              <b>
                <xsl:value-of select="."/>
              </b>
            </xsl:when>
      ...
      <td>
        <a class="one">
          <xsl:attribute name="name">_<xsl:value-of select="CatalogName"/></xsl:attribute>
        </a>
        <xsl:apply-imports/>
      </td>
    </tr>
  ...
  <xsl:template name="cat_ErrorTable">
    <tr>
      <td valign="top" align="left" width="150px">
        <b>
          <a class="one">
      ...
      <td>
        <xsl:for-each select="/Categorization">
          ...
          <xsl:call-template name="cat_ErrorList"/>
        </xsl:for-each>
      </td>
    </tr>
  </xsl:template>
  <xsl:template name="cat_ErrorList">
