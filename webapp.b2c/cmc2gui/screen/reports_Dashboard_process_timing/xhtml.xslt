...
<xsl:stylesheet version="2.0"
  ...
  <xsl:variable name="colorcodeRunning" select="'#00FF00'"/><!--Green-->
  <xsl:variable name="colorcodeSuspect" select="'#FFA500'"/><!--Orange-->
  <xsl:variable name="colorcodeError"   select="'#FF0000'"/><!--Red-->
  <xsl:variable name="colorcodeWarning" select="'#FF00FF'"/><!--Magenta-->
   ...
   <xsl:variable name="yLowerBound" select="number($yMax - /root/sql:rowset/sql:row[1]/sql:min_bound_minutes * ($yMax div  $maxRuntime))"/>
   <xsl:variable name="yUpperBound" select="number($yMax - /root/sql:rowset/sql:row[1]/sql:max_bound_minutes * ($yMax div  $maxRuntime))"/>
   ...
   <xsl:template match="/root">
    <html>
      <body>
        ...
        <p /><p />
        <hr />
        <p>Runtimes for the process : <strong><xsl:value-of select="sql:rowset/sql:row[1]/sql:process_name"/></strong>.
        </p>
        ...
        <br />
        The average runtime is calculated over de period Januari until Summer 2011, when the processes still ran on time.
        </p>
            <p>
               <script type="image/svg+xml">
                  <svg width="600px" height="450px" viewBox="-15 -15 440 335">
                     ...
                     <!-- Draw datasets -->
                     <xsl:call-template name="draw_countSets"/>
                  </svg>
               </script>
            </p>
        <br/>
          ...
          <xsl:when test="sql:rowset/sql:row">
            <table class="main">
              <tr>
                ...
                <td>Object count</td>
                <td>Runtime(m)</td>            
                <td>7Day average runtime(m)</td>
                <td>Longtime avg runtime(m)</td>
                <td>Lower bound(m)</td>
                <td>Upper bound(m)</td>
                <td>Objects/minute</td>
              </tr>
           ...
           <table border="1" cellspacing="0" cellpadding="0">
              ...
              <tr>
                 <td><xsl:attribute name="bgcolor" select="$colorcodeError"/>Red</td>
                 <td>Runtime is longer then the maximum for the process</td>
              </tr>
              <tr>
                 <td><xsl:attribute name="bgcolor" select="$colorcodeRunning"/>Green</td>
                 <td>Runtime is shorter then the minimum for the process</td>
              </tr>
              <tr>
                 <td><xsl:attribute name="bgcolor" select="$colorcodeSuspect"/>Orange</td>
                 <td>Runtime is between the two bounds</td>
              </tr>
           </table>
           <br/><br/>
           </p>
  ...
  <xsl:template match="sql:row">
    <tr>
      <xsl:variable name="checkDuration">
         <xsl:value-of select="f:compDuration(sql:runtime_minutes, sql:min_bound_minutes, sql:max_bound_minutes)"/>
      </xsl:variable>
      ...
      <td align="left"><xsl:attribute name="bgcolor" select="$bgcolor"/><xsl:value-of select="sql:day_of_week"/></td>
      <td align="right"><xsl:attribute name="bgcolor" select="$bgcolor"/><xsl:value-of select="sql:start_time"/></td>
      <td align="right"><xsl:attribute name="bgcolor" select="$bgcolor"/><xsl:value-of select="sql:object_count_out"/></td>
      <td align="right"><xsl:attribute name="bgcolor" select="$bgcolor"/><xsl:value-of select="sql:runtime_minutes"/></td>
      <td align="right"><xsl:attribute name="bgcolor" select="$bgcolor"/><xsl:value-of select="sql:moving_7day_average"/></td>
      <td align="right"><xsl:attribute name="bgcolor" select="$bgcolor"/><xsl:value-of select="sql:avg_runtime_minutes"/></td>
      <td align="right"><xsl:attribute name="bgcolor" select="$bgcolor"/><xsl:value-of select="sql:min_bound_minutes"/></td>
      <td align="right"><xsl:attribute name="bgcolor" select="$bgcolor"/><xsl:value-of select="sql:max_bound_minutes"/></td>
      <td align="right"><xsl:attribute name="bgcolor" select="$bgcolor"/><xsl:value-of select="sql:objects_per_minute"/></td>
    </tr>
  </xsl:template>
  ...
  <xsl:function name="f:compDuration" as="xs:string">
   ...
   <xsl:template name="draw_axis">
      <g style="stroke:black; stroke-width:0.25">
         ...
         <text x="{number(0.00*$xMax - 15)}" y="{number(0.50*$yMax)}" style="text-anchor:start">y-axis (minutes)</text>
   ...
   <xsl:template name="draw_datasets">
      ...
      <!-- Draw bounds -->
      <polyline style="fill:none;stroke:black;stroke-width:0.75" points="0,{$yLowerBound} {$xMax},{$yLowerBound}" />
      <polyline style="fill:none;stroke:black;stroke-width:0.75" points="0,{$yUpperBound} {$xMax},{$yUpperBound}" />
   </xsl:template>
   ...
   <!-- Draw our datasets... this involves drawing a legend entry,
     and plotting the points
     -->
   <xsl:template name="draw_countSets">
      <!-- create a colour for the dataset -->
      ...
      <xsl:for-each select="sql:rowset">
         ...
         <g style="stroke:{$colour}; stroke-width:0.5">
            <!-- draw datapoints -->
            <xsl:for-each select="sql:row">
               <xsl:call-template name="draw_count"/>
            </xsl:for-each>
         </g>
      </xsl:for-each>
   ...
   <xsl:template match="sql:row" mode="graph2">
      <xsl:variable name="x" select="(sql:day_of_year - $startDay) * ($xMax div  $maxDays)"/>
      <xsl:variable name="y_raw" select="sql:moving_7day_average * ($yMax div  $maxRuntime)"/>
      <xsl:variable name="y" select="$yMax - $y_raw"/>
      <xsl:value-of select="$x"/>,<xsl:value-of select="$y"/><xsl:text> </xsl:text>
   </xsl:template>
   ...
   <xsl:template match="sql:row" mode="graph3">
      <xsl:variable name="x" select="(sql:day_of_year - $startDay) * ($xMax div  $maxDays)"/>
      <xsl:variable name="y_raw" select="sql:avg_runtime_minutes * ($yMax div  $maxRuntime)"/>
      <xsl:variable name="y" select="$yMax - $y_raw"/>
      <xsl:value-of select="$x"/>,<xsl:value-of select="$y"/><xsl:text> </xsl:text>
   </xsl:template>
   ...
   <xsl:template name="draw_legend">
      ...
      <text x="{number(0.05*$xMax)}" y="{number(0.05 * $yMax + 10)}" style="text-anchor:start" fill ="{$colorcodeError}"> Runtime (minutes)</text>
      <text x="{number(0.05*$xMax)}" y="{number(0.05 * $yMax + 20)}" style="text-anchor:start" fill ="{$colorcodeRunning}"> Long time average (minutes)</text>
      <text x="{number(0.05*$xMax)}" y="{number(0.05 * $yMax + 30)}" style="text-anchor:start" fill ="{$colorcodeSuspect}"> 7 Day moving average (minutes)</text>
     <text x="{number(0.05*$xMax)}" y="{number(0.05 * $yMax + 40)}" style="text-anchor:start" fill ="blue"> Volume out (object count) </text>
   </xsl:template>
