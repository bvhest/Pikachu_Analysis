  ...
  <root>
	<xsl:attribute name="locale"><xsl:value-of select="$locale"/></xsl:attribute>
     <sql:execute-query xmlns:sql="http://apache.org/cocoon/SQL/2.0">
      <sql:query>
select lpad(id,5,0) as id,id as idReal,Name,Type,Location,Catalog,Pipeline,MachineAffinity,StartExec,EndExec,nvl(resultcode,0)resultcode
from Channels
order by lpad(id,5,0)
      </sql:query>
    </sql:execute-query>
  </root>
  </xsl:template>
