...
<xsl:stylesheet version="2.0"
   ...
   <xsl:template match="/root">
      <html>
         <body>
            <h1>Pikachu Performance Dashboard Object Timing Details</h1>
            <p />
            <p />
            <hr />
            <p>Object Timing for the process : <strong><xsl:value-of select="sql:rowset/sql:row[1]/sql:process_name"/></strong>.
            </p>
   ...
   <xsl:template name="draw_axis">
      <g style="stroke:black; stroke-width:0.25">
         ...
         <text x="{number(0.00*$xMax - 15)}" y="{number(0.50*$yMax)}" style="text-anchor:start">y-axis (obj/min)</text>
   ...
   <xsl:template name="draw_legend">
      ...
      <text x="{number(0.05*$xMax)}" y="{number(0.05 * $yMax + 10)}" style="text-anchor:start" fill ="{$colorcodeError}"> objects/minute</text>
      <text x="{number(0.05*$xMax)}" y="{number(0.05 * $yMax + 20)}" style="text-anchor:start" fill ="{$colorcodeRunning}"> Long time moving average (o/m)</text>
      <text x="{number(0.05*$xMax)}" y="{number(0.05 * $yMax + 30)}" style="text-anchor:start" fill ="{$colorcodeSuspect}"> 7 Day moving average (o/m)</text>
