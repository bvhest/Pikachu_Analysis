...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/octl">
    <source:write>
      <source:source><xsl:value-of select="concat($store_dir,'/AssetTemplateCatalog_',$doctimestamp,'_def.xml')"/></source:source>
      <source:fragment>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <catalog-definition o="AssetTemplateCatalog" l="none" ct="catalog_definition" DocTimeStamp="{$ts}">
              <xsl:for-each select="$incomingdocument/KeyValuePairs/object">
                <object o="{@object_id}"/>
              </xsl:for-each>
            </catalog-definition>
          </xsl:otherwise>
  ...
  <xsl:template match="catalog-definition">
    <xsl:copy copy-namespaces="no">
      ...
      <xsl:apply-templates select="node()"/>
      <xsl:for-each select="$incomingdocument/KeyValuePairs/object">
        <xsl:if test="empty($existing/objects/object[@o=current()/@object_id])">
          <object o="{@object_id}"/>
        </xsl:if>              
      </xsl:for-each>            
    </xsl:copy>
