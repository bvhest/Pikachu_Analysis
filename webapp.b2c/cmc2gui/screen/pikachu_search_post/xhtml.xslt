...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cinclude="http://apache.org/cocoon/include/1.0" xmlns:dir="http://apache.org/cocoon/directory/2.0" xmlns:osm="http://osmosis.gr/osml/1.0" xmlns:sql="http://apache.org/cocoon/SQL/2.0" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:me="http://apache.org/a">
      ...
      <body contentID="content">
        ...
        <form method="POST" enctype="multipart/form-data">
          ...
          <table>
            ...
            <tr><td style="width: 140px"></td><td style="width: 40px"></td></tr>            
            <tr>
        ...
        <h2>Search Result: <xsl:value-of select="$v_ctn"/></h2>
        <hr/>
        <br/>
        <xsl:apply-templates/>
      </body>
  ...
  <xsl:template match="/root/sql:rowset[@name = 'category']">
    ...
    <table  class="main">
      <tr>
        <td>Product CTN</td>
        <td>Raw&#160;content<a href="javascript:return false" title="Click for either raw xml content or preview content. This is 'PMT_Raw' content. No content will be available when status = 'PLACEHOLDER'.">?</a></td>
        <td>Enriched&#160;content<a href="javascript:return false" title="Click for either raw xml content or preview content. This is 'PMT_Enriched' content. No content will be available when status = 'PLACEHOLDER'.">?</a></td>
        <td>Catalog<a href="javascript:return false" title="The ATG catalog type to which the subcat belongs">?</a></td>
        <td>Group<a href="javascript:return false" title="The subcat's group">?</a></td>
        <td>Category<a href="javascript:return false" title="The subcat's category">?</a></td>
        <td>Subcat<a href="javascript:return false" title="The subcategory name. Click to see all product assignments for this subcat">?</a></td>
        <td>Master Leaflet<a href="javascript:return false" title="Click to retrieve the PSS in AEN for this CTN (note: this is not the GLL)">?</a></td>        
      </tr>
                <xsl:choose>
            ...
            <xsl:otherwise>
              <tr><td colspan="5" align="center"><xsl:attribute name="bgcolor">#FFD5D1</xsl:attribute>None found</td></tr>
            </xsl:otherwise>
          </xsl:choose>
    </table>
    <br />
  ...
  <xsl:template match="/root/sql:rowset[@name = 'catalogs']">
    ...
    <table class="main">
      ...
      <tr>
        <td>Product CTN</td>
        <td>Country<a href="javascript:return false" title="The country of assignment">?</a></td>
        <td>Locale<a href="javascript:return false" title="A locale within the country of assignment">?</a></td>        
        <td>Catalog<a href="javascript:return false" title="The catalog type of assignment">?</a></td>
        <td>Product Link<a href="javascript:return false" title="Click to be directed to the consumer site or FYP for the CTN and locale">?</a></td>
        <td>PDF<a href="javascript:return false" title="Click to retrieve the PSS for the CTN and locale">?</a></td>
        <td>SOP<a href="javascript:return false" title="SOP: Start Of Publication. Yellow: the SOP is in the future. Green: the SOP is in the past">?</a></td>
        <td>EOP<a href="javascript:return false" title="EOP: End Of Publication. Red: the EOP is in the past. Green: the EOP is in the future">?</a></td>
        <td>LastModified<a href="javascript:return false" title="Date of last modification of the LCB record (i.e. the country/catalog *assignment* NOT the product itself)">?</a></td>
        <td>Local going price</td>
        <td>Priority</td>
        <td>Deleted<a href="javascript:return false" title="Red: deleted = true. Green: deleted = false">?</a></td>
        <td>DeleteAfterDate<a href="javascript:return false" title="Red: non-empty. Green: empty">?</a></td>        
        <td>Division</td>
      </tr>
      <xsl:apply-templates/>
    </table>
  ...
  <xsl:template match="/root/sql:rowset[@name = 'catalogs']/sql:row">
    <tr>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="sql:catalog_type = 'FLAGSHIPSHOP'">
            <!-- er, go to consumer site (would like to go to flagship shop but don't have the links yet) -->
            <a target="_blank">
              <xsl:attribute name="href">
                <xsl:value-of select="concat('http://www.consumer.philips.com/consumer/',lower-case(substring(sql:locale,1,2)),'/',lower-case(substring(sql:locale,4,2)),'/consumer/cc/_productid_',translate(sql:ctn,'/','_'),'_',upper-case(substring(sql:locale,4,2)),'_',sql:catalog_type,'/')"/>                
              </xsl:attribute>Consumer site</a>
          </xsl:when>
          <xsl:when test="ends-with(sql:catalog_type,'_MARKETING')">
            <!-- go to FYP -->
            <a target="_blank">
              <xsl:attribute name="href">            
                <xsl:value-of select="concat('http://pww.findyourproduct.philips.com/ccrprd/f?p=402:7:::NO::P0_CTN:',translate(sql:ctn,'/','_'))"/>
              </xsl:attribute>FindYourProduct</a>                
          </xsl:when>
          <xsl:otherwise>
            <!-- go to consumer site -->
            <a target="_blank">
