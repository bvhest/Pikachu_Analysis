...
<xsl:stylesheet version="1.0" 
   ...
   <xsl:param name="file_name" select="/h:request/h:requestParameters/h:parameter[@name='file_name']/h:value" />
	...
	<xsl:template match="/root">
		<html>
			<body>
            <h2>Results for the translation cancelation</h2>
            <hr/>
            <br /><br />
            <xsl:apply-templates />
			</body>
      ...
      <p>
         <xsl:choose>
            <xsl:when test="sql:error">
               ...
               <strong><xsl:text>Cancellation was not succesfull!</xsl:text></strong>
               <br /><br />
<!-- for debugging purposes:
               <strong><xsl:text>Complete error: </xsl:text></strong>
               <br />
               <xsl:value-of select="sql:error"/>
               <br /><br />
               <strong><xsl:text>Substring error: </xsl:text></strong>
               <br />
...
-->               
            ...
            <xsl:otherwise>
               <xsl:text>Cancellation of the translation request </xsl:text>
