<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sql="http://apache.org/cocoon/SQL/2.0"> 
  ...
  <xsl:template match="referencedproducts">
    <xsl:variable name="filters-features"><features><xsl:for-each select="distinct-values(../../Filters/Purpose/Features/Feature/@code)"><feature code="{.}"/></xsl:for-each></features></xsl:variable>
    <xsl:variable name="filters-csitems"><csitems><xsl:for-each select="distinct-values(../../Filters/Purpose/CSItems/CSItem/@code)"><csitem code="{.}"/></xsl:for-each></csitems></xsl:variable>
    <xsl:copy>    
      <xsl:variable name="o" select="object/@id"/>
  ...
  <xsl:template match="Product" mode="referencedproduct">
    ...
    <xsl:variable name="o" select="CTN"/>
    <xsl:copy>    
      <xsl:copy-of select="@*"/>
