...
<xsl:stylesheet version="2.0"
  ...
  <xsl:variable name="jobs_schedule_id" select="'775'"/>       
          ...
          <result>
            <html>
              <body>
                ...
                <p>
                  File <b><xsl:value-of select="$filename"/>.xml</b> was saved in the <i><xsl:value-of select="$ct"/></i> inbox.
                </p>
                  <xsl:text>If all files have been uploaded, the next step is to schedule the jobs that need to run to process this content.
Press the button below, and then on the following page press FIRE NOW</xsl:text><br/><br/><br/>
            <button>
                    ...
                    <xsl:text>Schedule </xsl:text><xsl:value-of select="$ct"/><xsl:text> load</xsl:text>
            </button>                       
                  <br/><br/><br/>
                  <hr/>                
              </body>
