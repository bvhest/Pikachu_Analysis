...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/octl">
    <source:write>
      <source:source><xsl:value-of select="concat($store_dir,'/LocalizationCatalog_',$doctimestamp,'_def.xml')"/></source:source>
      <source:fragment>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <catalog-definition o="LocalizationCatalog" l="none" ct="catalog_definition" DocTimeStamp="{$ts}">
              <xsl:for-each select="$incomingdocument/Localizations/Localization">
                <object o="{@code}"/>
              </xsl:for-each>
            </catalog-definition>
          </xsl:otherwise>
