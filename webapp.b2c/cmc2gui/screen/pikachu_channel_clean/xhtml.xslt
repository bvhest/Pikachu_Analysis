		...
		<html>
			<body contentID="content">
				...
				<xsl:if test="$param1 != ''">
					...
					<p>In addition it means that the next export is a full export, because the information needed to find the delta is deleted.</p>
					<br/>
					<a>
						<xsl:attribute name="href"><xsl:value-of select="concat($gui_url, $sectionurl, 'pikachu_channel_clean_real/', $param1, '?channel=',$channel)"/></xsl:attribute>Continue</a>
				</xsl:if>
			</body>
		</html>
	</xsl:template>
